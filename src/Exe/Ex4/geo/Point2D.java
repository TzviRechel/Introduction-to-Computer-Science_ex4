...
public class Point2D{
	...
	public void scale(Point2D cen, double ratio) {
		...
		Point2D vec = new Point2D(this.vector(cen));
		this._x += (1-ratio)*vec.x();
		this._y += (1-ratio)*vec.y();
	}
	
	public void rotate(Point2D cen, double angleDegrees) {
	        //https://stackoverflow.com/questions/22491178
	        double x1 = this.x() - cen.x();
	        double y1 = this.y() - cen.y();

	        double x2 = x1 * Math.cos(angleDegrees) - y1 * Math.sin(angleDegrees);
	        double y2 = x1 * Math.sin(angleDegrees) + y1 * Math.cos(angleDegrees);
